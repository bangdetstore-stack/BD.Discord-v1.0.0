...
export class BuyProductUserInterface extends MessageUserInterface {
    ...
    protected override initComponents(): void {
        ...
        const buyRupiahButton = new ExtendedButtonComponent(
            {
                customId: `${this.id}+buy-rupiah`,
                label: '💳 Bayar Rupiah (QRIS)',
                emoji: {name: '💳'},
                style: ButtonStyle.Primary,
                time: 120_000,
            },
            (interaction: ButtonInteraction) => this.handleBuyWithRupiah(interaction)
        )

        this.components.set(selectProductMenu.customId, selectProductMenu)
        this.components.set(buyButton.customId, buyButton)
        this.components.set(buyRupiahButton.customId, buyRupiahButton)
    ...
    protected override updateComponents(): void {
        ...
        const buyRupiahButton = this.components.get(`${this.id}+buy-rupiah`)
        if (buyRupiahButton instanceof ExtendedButtonComponent) {
            buyRupiahButton.toggle(this.selectedProduct != null)
        }
    }

    private async handleBuyWithRupiah(interaction: ButtonInteraction): Promise<void> {
        if (!this.selectedProduct) {
            await updateAsErrorMessage(interaction, "Pilih produk terlebih dahulu!")
            return
        }

        await startPaymentFlow(interaction, this.selectedShop, this.selectedProduct)
    }
