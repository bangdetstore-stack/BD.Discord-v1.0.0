...
async function setup() {
    ...
    await saveConfig()
    PrettyLog.success('Configuration saved to .env', false)
...
async function saveConfig() {
    ...
    const lines = envContent.split('\n').filter(line => !line.startsWith('DISCORD_TOKEN=') && !line.startsWith('DISCORD_CLIENT_ID='))
    
    lines.push(`DISCORD_CLIENT_ID=${config.clientId}`)
    lines.push(`DISCORD_TOKEN=${config.token}`)
